...
dst_dir = os.path.join(os.path.dirname(src_dir), "positive_scorebugs_test_quarterres")
...
for fname in image_files:
    ...
    with Image.open(src_path) as img:
        new_size = (max(1, img.width // 4), max(1, img.height // 4))
        img_resized = img.resize(new_size, Image.LANCZOS)
        img_resized.save(dst_path)
    print(f"Saved {dst_path} ({new_size[0]}x{new_size[1]})")

print(f"All images downscaled to quarter resolution and saved to {dst_dir}")
